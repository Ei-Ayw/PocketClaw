...
impl Agent {
    ...
    pub fn from_config(config: &Config) -> Result<Self> {
        ...
        Self::from_config_and_observer(config, observer)
    }

    pub fn from_config_and_observer(config: &Config, observer: Arc<dyn Observer>) -> Result<Self> {
            ...
            match tool.execute(call.arguments.clone()).await {
                ...
                Err(e) => {
                    ...
                    self.observer.record_event(&ObserverEvent::ToolCall {
                        ...
                        duration: start.elapsed(),
                        success: false,
                        output: Some(err_msg.clone()),
                    });
                    format!("Error executing {}: {err_msg}", call.name)
                }
            }
        } else {
