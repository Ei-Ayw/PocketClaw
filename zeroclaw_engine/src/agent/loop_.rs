...
pub(crate) async fn run_tool_call_loop(
    ...
    for iteration in 0..max_iterations {
        ...
        let chat_result = if provider.supports_streaming() && !use_native_tools {
            // Use streaming for text-only responses to enable real-time UI feedback
            let mut full_text = String::new();
            let mut stream = provider.stream_chat_with_history(
                &prepared_messages.messages,
                model,
                temperature,
                providers::StreamOptions::new(true),
            );
                ...
                match chunk_result {
                    Ok(chunk) => {
                        if !chunk.delta.is_empty() {
                            ...
                            // Report delta to observer for real-time UI mapping
                            observer.record_event(&ObserverEvent::TextDelta {
                                delta: chunk.delta.clone(),
                            });
                        }
                    }
                    Err(e) => {
                        error = Some(anyhow::anyhow!("Streaming error: {}", e));
                        break;
                    }
                }
            }
